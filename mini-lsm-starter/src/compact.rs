...
impl LsmStorageInner {
    fn compact_generate_sst_from_iter_v2(
        &self,
    ...
    fn compact_v2(&self, task: &CompactionTask) -> Result<Vec<Arc<SsTable>>> {
        let snapshot = {
    ...
    fn trigger_compaction_v2(&self) -> Result<()> {
        let snapshot = {
            let state = self.state.read();
            state.clone()
        };
    ...
    //TODO(xingyu): visit this one later
